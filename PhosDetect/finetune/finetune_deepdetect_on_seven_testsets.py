...
class PeptideDataset(Dataset):
    """肽序列数据集 - 使用DeepDetect的编码方式"""
    
    def __init__(self, sequences, labels):
        self.sequences = sequences
        self.labels = labels
        
        # DeepDetect的词汇表（21个字符：20个氨基酸 + 1个padding）
        self.vocab = {
            '0': 0,  # padding
    ...
    def finetune_on_seven_testsets(self):
        ...
        for test_file in test_sets:
            ...
            if best_auc > initial_auc:
                model_save_path = os.path.join(output_dir, f'deepdetect_finetuned_{test_name}_auc{best_auc:.4f}.pth')
                finetuned_model.save_model(model_save_path)
                print(f"  ✅ Performance improved, model saved to: {model_save_path}")
            else:
                model_save_path = os.path.join(output_dir, f'deepdetect_finetuned_{test_name}_no_improvement.pth')
                finetuned_model.save_model(model_save_path)
...
def main():
    parser = argparse.ArgumentParser(description='Finetune DeepDetect model on seven test sets')
    parser.add_argument('--pretrained_model', type=str, 
                       default='/data0/wangb/cd/duibi0826/0826comparison/model_weights/deepdetect_best.pth',
                       help='Path to pretrained DeepDetect model')
    parser.add_argument('--device', type=str, default='cuda', help='Device to use (cuda/cpu)')
    
    args = parser.parse_args()
    
    # 创建微调器
    finetuner = DeepDetectFinetuner(args.pretrained_model, args.device)
