...
class PeptideDataset(Dataset):
    """肽序列数据集 - 使用PhosSight的编码方式"""
    
    def __init__(self, sequences, labels):
        self.sequences = sequences
        self.labels = labels
        
        # PhosSight的词汇表（24个字符：20个氨基酸 + 3个磷酸化 + 1个padding）
        self.vocab = {
            'Z': 0,  # padding
            'A': 1, 'C': 2, 'D': 3, 'E': 4, 'F': 5, 'G': 6, 'H': 7, 'I': 8, 'K': 9,
            'L': 10, 'M': 11, 'N': 12, 'P': 13, 'Q': 14, 'R': 15, 'S': 16, 'T': 17,
            'V': 18, 'W': 19, 'Y': 20,
            's': 21, 't': 22, 'y': 23  # 磷酸化氨基酸
        }
...
class PhosSightFinetuner:
    ...
    def finetune_model(self, model, dataloader, sequences, labels, test_name):
        ...
        for epoch in range(self.finetune_config['epochs']):
            ...
            for batch_sequences, batch_labels in tqdm(dataloader, desc=f"Epoch {epoch+1}"):
                batch_sequences = batch_sequences.to(self.device)
    ...
    def finetune_on_three_testsets(self):
        """在三个测试集上微调模型"""
        ...
        for test_file in test_sets:
            ...
            if best_auc > initial_auc:
                model_save_path = os.path.join(output_dir, f'phosight_finetuned_{test_name}_auc{best_auc:.4f}.pth')
                torch.save(finetuned_model.state_dict(), model_save_path)
                print(f"  ✅ Performance improved, model saved to: {model_save_path}")
            else:
                model_save_path = os.path.join(output_dir, f'phosight_finetuned_{test_name}_no_improvement.pth')
                torch.save(finetuned_model.state_dict(), model_save_path)
...
def main():
    parser = argparse.ArgumentParser(description='Finetune PhosSight model on three test sets')
    parser.add_argument('--pretrained_model', type=str, 
                       default='/data0/wangb/cd/duibi0826/0826comparison/model_weights/phosight_v2_best.pth',
                       help='Path to pretrained PhosSight model')
    parser.add_argument('--device', type=str, default='cuda', help='Device to use (cuda/cpu)')
    
    args = parser.parse_args()
    
    # 创建微调器
    finetuner = PhosSightFinetuner(args.pretrained_model, args.device)
    
    # 开始微调
    results = finetuner.finetune_on_three_testsets()
