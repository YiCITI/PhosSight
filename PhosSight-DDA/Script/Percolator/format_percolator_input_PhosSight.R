...
output <- 'E:/MyProject/PTM-Project/datasets/ExampleData1/PXD000138/Percolator/DeepRescore2WithoutDLFeatures.pin'
software='maxquant'
phosphors <- fread('E:/MyProject/PTM-Project/datasets/ExampleData1/PXD000138/PhosphoRS/PhosphoRS.txt')
pdeep_phospho <- fread('E:/MyProject/PTM-Project/datasets/ExampleData1/PXD000138/pDeep3_Results/pDeep3PredictionResults.Phospho.txt')
pdeep_nonphospho <- fread('E:/MyProject/PTM-Project/datasets/ExampleData1/PXD000138/pDeep3_Results/pDeep3PredictionResults.nonPhospho.txt')
autort_phospho <- fread('E:/MyProject/PTM-Project/datasets/ExampleData1/PXD000138/autoRT_Results/tf_prediction/phospho.prediction.tsv')
autort_nonphospho <- fread('E:/MyProject/PTM-Project/datasets/ExampleData1/PXD000138/autoRT_Results/tf_prediction/nonPhospho.prediction.tsv')
...
phosSight_phospho <- fread('E:/MyProject/PTM-Project/datasets/ExampleData1/PXD000138/phosSight_Results/pth_prediction/PhosSight.Predict.Phospho.txt')
phosSight_nonphospho <- fread('E:/MyProject/PTM-Project/datasets/ExampleData1/PXD000138/phosSight_Results/pth_prediction/PhosSight.Predict.nonPhospho.txt')
} else {
args <- commandArgs(T)
...
FixedMods <- args[11]
phosSight_phospho <- fread(args[12])
phosSight_nonphospho <- fread(args[13])
}
...
phosSight_phospho$peptide <- NULL
colnames(phosSight_phospho)[1] <- 'Title'
colnames(phosSight_phospho)[3] <- 'Detectability'
phosSight_phospho <- left_join(phosphors, phosSight_phospho, by = c("Spectrum.Name" = "Title", "IsoformSequence" = "IsoformSequence")) %>% select(Spectrum.Name, IsoformSequence,Detectability)
colnames(phosSight_phospho)[1] <- 'Title'
...
if (FixedMods!='null'){
  for (i in 1:length(FixedModsInfo)){
    ...
    if (name!='Phospho'){
      phosSight_phospho$IsoformSequence <- gsub(aa,number,phosSight_phospho$IsoformSequence)
    }
  }
}

# cat("=== DEBUG: phosSight_phospho columns ===\n")
# cat("phosSight_phospho columns:", paste(colnames(phosSight_phospho), collapse=", "), "\n")
# cat("phosSight_phospho dimensions:", dim(phosSight_phospho), "\n")
...
for (i in 1:length(VariableModsInfo)){
  ...
  sym = tmp2[5]
  if (name!='Phospho'){
    phosSight_nonphospho$IsoformSequence <- gsub(paste0(aa,sym),number,phosSight_nonphospho$IsoformSequence)
  }
}
if (FixedMods!='null'){
  for (i in 1:length(FixedModsInfo)){
...
#phosSight_nonphospho$IsoformSequence <- str_replace_all(phosSight_nonphospho$IsoformSequence,c("M1" = "1", "C3" = "5"))
phosSight_nonphospho$peptide <- NULL
colnames(phosSight_nonphospho)[1] <- 'Title'
colnames(phosSight_nonphospho)[3] <- 'Detectability'

phosSight <- merge(phosSight_phospho,phosSight_nonphospho,all = TRUE)

# cat("=== DEBUG: phosSight columns ===\n")
# cat("phosSight columns:", paste(colnames(phosSight), collapse=", "), "\n")
# cat("phosSight dimensions:", dim(phosSight), "\n")
...
input_features$PhosSightProb <- NULL
input_features$IsoformSequence_PhosSight <- NULL
input_features$IsoformSequence_autort_pDeep <- NULL

colnames(input_features)[3] <- "CalcMass"
colnames(input_features)[ncol(input_features)-2] <- "ModifiedPeptide"
...
input_features <- left_join(input_features, phosSight, by = c('Title' = 'Title', 'ModifiedPeptide' = 'IsoformSequence'))

input_features[is.na(input_features$entropy),]$entropy <- 0
input_features[is.na(input_features$Detectability),]$Detectability <- 0
...
input_features$autort_pDeep_Prob <- NULL
input_features$autort_pDeep_phosSight_Prob <- NULL


# cat("=== DEBUG: input_features columns ===\n")
# cat("input_features columns:", paste(colnames(input_features), collapse=", "), "\n")
# cat("input_features dimensions:", dim(input_features), "\n")


colnames(input_features)[ncol(input_features)-2] <- 'similarity'
colnames(input_features)[ncol(input_features)-1] <- 'detectability'
input_features <- input_features %>% select(-similarity, -detectability, similarity, detectability)
...
# output_format$Charge <- NULL
# output_format$ratio <- NULL
# output_format$similarity <- NULL
# output_format$detectability <- NULL
