...
import random
from model import biGRU_Detect, Transformer_Detect
...
def split_data(path, batch_size=32, val_size=0.05, test_size=0.05, max_len=53):
    ...
    data.iloc[:,0] = data.iloc[:,0].apply(lambda x: x[:max_len] if len(x) > max_len else x.ljust(max_len, 'Z')) # padding with Z
...
def Train(args, model, train_loader, val_loader, learning_rate, num_epochs, model_path, patience=10, logger=None):
    ...
    for epoch in range(num_epochs):
        ...
        logger.info('-'*30 + f' Epoch {epoch+1}/{num_epochs} ' + '-'*30)
    ...
    csv_path = os.path.join(log_dir, f'{type(model).__name__}_detect_training_log_{timestamp}.csv')
...
def Test(model, test_loader, weight_path, logger=None):
    """Test the model."""
    if logger is None:
        logger = logging.getLogger(__name__)
...
def create_model(model_type, **kwargs):
    ...
    Args:
        model_type: 'bigru' or 'transformer'
    ...
    if model_type.lower() == 'bigru':
        ...
        model = Transformer_Detect(
            vocab_size=kwargs.get('vocab_size', 24),
        ...
        raise ValueError(f"Unknown model type: {model_type}. Supported types: 'bigru', 'transformer'")
...
def parse_args():
    ...
    parser.add_argument('-e', '--epochs', type=int, default=30, help='Number of epochs for training.')  # Reduced from 100
    parser.add_argument('-lr', '--learning_rate', type=float, default=0.0003, help='Learning rate for the optimizer.')  # Reduced from 0.001
    ...
    parser.add_argument('--model_type', type=str, default='bigru', choices=['bigru', 'transformer'], help='Model type: bigru or transformer')
    parser.add_argument('--log_level', type=str, default='INFO', choices=['DEBUG', 'INFO', 'WARNING', 'ERROR'], help='Logging level')
    parser.add_argument('--log_file', type=str, default='./log.txt', help='Log file path (optional)')
    ...
    parser.add_argument('--max_length', type=int, default=64, help='Maximum sequence length for Transformer')
...
if __name__ == "__main__":
    ...
    Train(args, model, train_loader, val_loader, learning_rate, num_epochs, model_path, patience, logger)
    model_path = args.model_path

    test_loss, test_acc, test_f1, test_precision, test_recall, test_auc = Test(model, test_loader, model_path, logger)
