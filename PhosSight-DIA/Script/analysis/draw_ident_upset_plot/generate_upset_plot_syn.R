# Create UpSet plot for syn dataset using CSV data
...
if (length(args) < 2) {
  cat("Usage: Rscript generate_upset_plot_syn.R --peptides_input <path> --psms_input <path> --output_dir <path> --dataset_name <name>\n")
  cat("Example: Rscript generate_upset_plot_syn.R --peptides_input upset_plot_JPST000859_finetuned_peptides.csv --psms_input upset_plot_JPST000859_finetuned_phos_PSMs.csv --output_dir ./output/ --dataset_name syn\n")
...
generate_upset_plot <- function(csv_data_path, output_suffix, plot_title) {
  ...
  # Set row names to PSM/peptide identifiers for reference
...
peptides_output <- generate_upset_plot(peptides_input, "peptides_upset", "Peptides")

# PSMs upset plot
cat("\nGenerating PSMs upset plot...\n")
psms_output <- generate_upset_plot(psms_input, "psms_upset", "PSMs")
