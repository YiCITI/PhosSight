...
def batch_run_diann_two_runs(
    ...
    for timeh in timehs:
        final_output_dir = output_dir / "for_finetuning_res" / f"{timeh}h_rep1"
        if not os.path.exists(final_output_dir):
            os.makedirs(final_output_dir)

        raw_path = raw_dir / f"20250306_A549_pho_{timeh}h_rep1.raw"
        spec_lib_parquet_path = spec_lib_dir / "spec_library_original.parquet"
        output_path = final_output_dir / "report.parquet"
        param = f'--f "{raw_path}" --lib "{spec_lib_parquet_path}" --threads 32 --verbose 1 --out "{output_path}" --qvalue 0.01 --matrices  --temp "{final_output_dir}"  --unimod4 --var-mods 1 --var-mod UniMod:21,79.966331,STY --window 10 --mass-acc 10 --mass-acc-ms1 4 --peptidoforms --reanalyse --rt-profiling'
...
def batch_run_diann_original(
    ...
    for timeh in ["01", "24"]:
        for replicate in ["1", "2", "3", "4"]:
            raw_file = raw_dir / f"20250306_A549_pho_{timeh}h_rep{replicate}.raw"
            raw_files.append(f'--f "{raw_file}"')
    
    raw_files_param = " ".join(raw_files)
    
    param = f'{raw_files_param} --lib "{spec_lib_parquet_path}" --threads 32 --verbose 1 --out "{output_path}" --qvalue 0.01 --matrices  --temp "{final_output_dir}"  --unimod4 --var-mods 1 --var-mod UniMod:21,79.966331,STY --window 10 --mass-acc 10 --mass-acc-ms1 4 --peptidoforms --reanalyse --rt-profiling'
    cmd = f"{diann_cmd_prefix} {param}"
    print(f"Running command: {cmd}")
    result = subprocess.run(cmd, shell=True, capture_output=False, text=True)
    print("Return code:", result.returncode)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--step', type=str, choices=['two_rep', 'original', 'pretrained', 'finetuned'], 
                        required=True, help='Choose the step to run: 1 (three runs), 2 (pretrained filtering), 3 (fine-tuned filtering)')
    parser.add_argument('--diann_cmd_prefix', type=str, required=True,
                        help='The command prefix to run DIA-NN (inside Singularity container)')
    parser.add_argument('--output_dir', type=str, required=True,
                        help='The output directory for DIA-NN results')
    ...
    if args.step == 'two_rep':
        print("Running three DIA-NN analyses...")
        batch_run_diann_two_runs(output_dir, raw_dir, spec_lib_dir, singularity_diann_cmd_prefix)
