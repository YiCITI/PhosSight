# Generate fasta databases for JPOST000859 synthetic peptides and yeast peptides,
# and also combine with E.coli peptides for entrapment proteins
...
# work_dir = path.join(path.dirname(path.abspath(__file__)), 'JPST000859')

def step1_prepare_all_peptides(work_dir):
    """
    Step 1: Prepare all possible peptides
    Generate peptide files from synthetic peptides, yeast, E. coli, and caster proteomes and combine them
    """
    print("=== Step 1: Prepare all possible peptides ===")
    
    print("Generating modified peptide FASTA file...")
    generate_modified_pep_fasta_file(
        file_path=path.join(work_dir, 'uniprotkb_proteome_UP000002311_2025_07_07.fasta'),
        output_file=path.join(work_dir, 'yeast_all_peptides_2_7_46.fasta'),
        miss_cleavage=2,
        min_length=7,
        max_length=46,
    ...
    generate_modified_pep_fasta_file(
        file_path=path.join(work_dir, 'uniprotkb_proteome_UP000000625_2025_09_10.fasta'),
        output_file=path.join(work_dir, 'ecoli_all_peptides_2_7_46.fasta'),
        miss_cleavage=2,
        min_length=7,
        max_length=46,
        is_replace_STY_to_BJX=False
    ...
    generate_modified_pep_fasta_file(
        file_path=path.join(work_dir, 'uniprotkb_proteome_UP000008311_2025_09_18.fasta'),
        output_file=path.join(work_dir, 'castor_all_peptides_2_7_46.fasta'),
        miss_cleavage=2,
        min_length=7,
        max_length=46,
        is_replace_STY_to_BJX=False
    ...
    combine_fasta(fasta1_path=path.join(work_dir, 'combined_syn_yeast_original.fasta'),
                  fasta2_path=path.join(work_dir, 'ecoli_all_peptides_2_7_46.fasta'),
                  output_path=path.join(work_dir, 'combined_syn_yeast_ecoli_original.fasta'))
    combine_fasta(fasta1_path=path.join(work_dir, 'combined_syn_yeast_ecoli_original.fasta'),
                  fasta2_path=path.join(work_dir, 'castor_all_peptides_2_7_46.fasta'),
                  output_path=path.join(work_dir, 'combined_syn_yeast_ecoli_castor_original.fasta'))
