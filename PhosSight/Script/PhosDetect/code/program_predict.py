...
def prepare_prediction_data(path, batch_size, max_len=53):
    ...
    isoform_sequence = data['IsoformSequence']
    peptides = data['peptide']
    # max_len = max(peptides.apply(len))
    peptides_padding = peptides.apply(lambda x: x[:max_len] if len(x) > max_len else x.ljust(max_len, 'Z')) # padding with Z
    ...
    return prediction_loader, spectrum.tolist(), isoform_sequence.tolist(), peptides.tolist(), peptides_padding.tolist()  # Return both loader and original sequences
...
def save_predictions_to_csv(output_path, spectrum, isoform_sequence, sequences, predictions):
    ...
    results_df = pd.DataFrame({
        'Spectrum': spectrum,
        'IsoformSequence': isoform_sequence,
        'peptide': sequences,
...
def main():
    ...
    # Load model
    print("Loading model...")
    model = Detect(in_features=10, out_features=20, num_layers=2, dropout=0.3)
    ...
    prediction_loader, spectrum, isoform_sequence, sequences, sequences_padding = prepare_prediction_data(args.path, batch_size=args.batch_size, max_len=args.max_len)
    ...
    save_predictions_to_csv(args.output, spectrum, isoform_sequence, sequences, predictions)
