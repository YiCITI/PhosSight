...
# 读取 GSB 数据
df = pd.read_csv(DATA_DIR / "Rice_phospho_GSB.csv", low_memory=False)
print(f"总记录数: {len(df)}")
...
# 生成负样本
negative_peptides = {}
with open(DATA_DIR / "Rice.fasta", 'r') as f:
    sequence = ''
    for line in f:
        if line.startswith('>'):
            if sequence:
...
test_df.to_csv(DATA_DIR / "rice_test.csv", index=False)
print(f"测试集: {len(test_df)} (正: {max_samples}, 负: {max_samples})")

pd.DataFrame({'peptide': positive_peptides, 'label': 1}).to_csv(DATA_DIR / "rice_positive.csv", index=False)
pd.DataFrame({'peptide': negative_peptides[:100000], 'label': 0}).to_csv(DATA_DIR / "rice_negative.csv", index=False)
print("完成!")
