# R脚本：将MASCI定量结果与PhosphoRS鉴定结果匹配（17 plex完整版）
...
# PhosphoRS鉴定结果文件（需要先合并所有批次的鉴定结果）
pathin2 = 'PhosphoRSResults_TMT01_17.txt'

# 输出文件
pathout = 'PhosphoRS_Intensity_full.txt'

# ===== 读取PhosphoRS鉴定结果 =====
cat("========================================\n")
cat("Step 1: 读取PhosphoRS鉴定结果（TMT01-17）\n")
cat("========================================\n\n")
...
PhosphoRS <- fread(pathin2)

# 选择需要的列
cols_to_select <- c("Title", "Charge", "Peptide", "Proteins")

if ("IsoformSequence_PhosphoRS" %in% colnames(PhosphoRS)) {
  cols_to_select <- c(cols_to_select, "IsoformSequence_PhosphoRS")
} else if ("Mod_Sequence_for_phosphoRS" %in% colnames(PhosphoRS)) {
  cols_to_select <- c(cols_to_select, "Mod_Sequence_for_phosphoRS")
}

if ("Modifications_abbrev" %in% colnames(PhosphoRS)) {
  cols_to_select <- c(cols_to_select, "Modifications_abbrev")
} else if ("Modification" %in% colnames(PhosphoRS)) {
  cols_to_select <- c(cols_to_select, "Modification")
}

PhosphoRS <- PhosphoRS %>% select(all_of(cols_to_select))

# 构建Title2用于匹配
cat("构建Title2用于匹配...\n")
PhosphoRS$Title2 <- as.character(sapply(PhosphoRS$Title, function(x) {
  parts <- strsplit(x, '[.]')[[1]]
  if (length(parts) >= 3) {
...
PhosphoRS$Plex <- sapply(PhosphoRS$Title, function(x) {
  prefix <- substr(x, 1, 2)
...
cat("PhosphoRS总记录数:", nrow(PhosphoRS), "\n\n")
cat("各批次记录数:\n")
for (plex in plex_list) {
  count <- sum(PhosphoRS$Plex == plex, na.rm = TRUE)
...
if ("IsoformSequence_PhosphoRS" %in% colnames(Results)) {
  output_cols <- c(output_cols, 'IsoformSequence_PhosphoRS')
